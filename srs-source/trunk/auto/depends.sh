...
# Check optional tools for QUIC (ngtcp2)
if [[ $SRS_QUIC == YES ]]; then
    echo "Checking optional tools for QUIC: autoconf automake libtool"
    check_command "autoconf --version" "autoconf" "autoconf" "autoconf" "autoconf"
    check_command "automake --version" "automake" "automake" "automake" "automake"
    # macOS 系统的 libtool 不支持 --version，尝试 glibtool (GNU libtool)
    if ! check_command "glibtool --version" "libtool" "libtool" "libtool" "libtool"; then
        # 如果 glibtool 也失败，尝试直接检查 libtool 是否存在（忽略版本检查）
        if command -v libtool >/dev/null 2>&1 || command -v glibtool >/dev/null 2>&1; then
            echo "Note: libtool found (version check may fail on macOS)"
        else
            echo "Error: libtool (or glibtool) not found. Please install: brew install libtool"
            exit 1
        fi
    fi
fi
...
if [[ $SRS_SSL == YES && $SRS_USE_SYS_SSL != YES ]]; then
    ...
    if [[ $SRS_SSL_1_0 == YES ]]; then
        ...
        # 使用 QuicTLS (OpenSSL 1.1.1 + QUIC 支持) 替代 OpenSSL 1.1.1
        OPENSSL_CANDIDATE="openssl-quictls-fit" &&
...
#####################################################################################
#####################################################################################
# ngtcp2 (QUIC库) - 使用 QuicTLS 作为 crypto 后端
#####################################################################################
if [[ $SRS_QUIC == YES ]]; then
    # 确保 QuicTLS 已构建（QuicTLS = OpenSSL 1.1.1 + QUIC 支持）
    if [[ $SRS_SSL != YES || $SRS_USE_SYS_SSL == YES ]]; then
        echo "Error: QUIC requires QuicTLS. Please enable SSL with --ssl=on and do not use --use-sys-ssl"
        exit -1
    fi
    
    if [[ -f ${SRS_OBJS}/${SRS_PLATFORM}/3rdparty/ngtcp2/lib/libngtcp2.a ]]; then
        rm -rf ${SRS_OBJS}/ngtcp2 && cp -rf ${SRS_OBJS}/${SRS_PLATFORM}/3rdparty/ngtcp2 ${SRS_OBJS}/ &&
        echo "ngtcp2 is ok."
    else
        if [[ ! -f ${SRS_OBJS}/${SRS_PLATFORM}/3rdparty/openssl/lib/libssl.a ]]; then
            echo "Error: OpenSSL not found. QUIC requires OpenSSL to be built first."
            exit -1
        fi
        
        echo "Build ngtcp2" &&
        rm -rf ${SRS_OBJS}/${SRS_PLATFORM}/ngtcp2-fit ${SRS_OBJS}/${SRS_PLATFORM}/3rdparty/ngtcp2 ${SRS_OBJS}/ngtcp2 &&
        cp -rf ${SRS_WORKDIR}/3rdparty/ngtcp2-fit ${SRS_OBJS}/${SRS_PLATFORM}/ &&
        (
            cd ${SRS_OBJS}/${SRS_PLATFORM}/ngtcp2-fit &&
            # 初始化git submodules
            ...
            # 配置ngtcp2，使用QuicTLS作为crypto后端
            # QuicTLS = OpenSSL 1.1.1 + QUIC 支持
            OPENSSL_CFLAGS="-I${SRS_DEPENDS_LIBS}/${SRS_PLATFORM}/3rdparty/openssl/include"
            OPENSSL_LIBS="-L${SRS_DEPENDS_LIBS}/${SRS_PLATFORM}/3rdparty/openssl/lib -lssl -lcrypto"
            PKG_CONFIG_PATH="${SRS_DEPENDS_LIBS}/${SRS_PLATFORM}/3rdparty/openssl/lib/pkgconfig:${PKG_CONFIG_PATH:-}"
            
            # --with-openssl 会自动检测 QuicTLS
            ./configure --prefix=${SRS_DEPENDS_LIBS}/${SRS_PLATFORM}/3rdparty/ngtcp2 \
                       --enable-static \
                       --disable-shared \
                       --without-libnghttp3 \
                       --without-ev-lib \
            ...
            # 设置编译环境变量（让子目录也能找到OpenSSL）
            export OPENSSL_CFLAGS="${OPENSSL_CFLAGS}"
            export OPENSSL_LIBS="${OPENSSL_LIBS}"
            export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}"
            make ${SRS_JOBS} &&
            make install &&
            # 确保crypto库也被复制到安装目录（QuicTLS后端）
            if [ -f crypto/quictls/.libs/libngtcp2_crypto_quictls.a ]; then
                mkdir -p ${SRS_DEPENDS_LIBS}/${SRS_PLATFORM}/3rdparty/ngtcp2/lib
                cp crypto/quictls/.libs/libngtcp2_crypto_quictls.a ${SRS_DEPENDS_LIBS}/${SRS_PLATFORM}/3rdparty/ngtcp2/lib/
            fi
        ) &&
        # 复制整个ngtcp2目录（包括crypto库）到最终位置
        cp -rf ${SRS_OBJS}/${SRS_PLATFORM}/3rdparty/ngtcp2 ${SRS_OBJS}/ &&
        # 验证crypto库已正确复制
        if [ ! -f ${SRS_OBJS}/ngtcp2/lib/libngtcp2_crypto_quictls.a ]; then
            echo "Error: Failed to copy ngtcp2 crypto library (QuicTLS)"
            exit 1
        fi &&
        echo "ngtcp2 is ok."
    fi
