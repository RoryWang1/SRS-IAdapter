...
#include <srs_app_caster_myproto.hpp>
#include <srs_app_caster_quic_fec_ts.hpp>
...
SrsServer::SrsServer()
{
    ...
    adapter_api_handler_ = new SrsAdapterHttpApiHandler();
    ppid_ = ::getppid();
    ...
    stream_caster_myproto_ = NULL; // 将在需要时创建
...
SrsServer::~SrsServer()
{
    ...
    srs_freep(adapter_api_handler_);
    ...
    srs_freep(stream_caster_myproto_);
...
#endif
        } else if (srs_stream_caster_is_myproto(caster)) {
            // 创建MyProto caster listener实例
            stream_caster_myproto_ = srs_create_myproto_caster_listener(this, conf);
            if (!stream_caster_myproto_) {
                return srs_error_new(ERROR_STREAM_CASTER_ENGINE, "Failed to create myproto caster listener");
            }
            
            listener = stream_caster_myproto_;
        } else if (srs_stream_caster_is_quic_fec_ts(caster)) {
            // 创建QUIC+FEC TS caster listener实例
            stream_caster_myproto_ = srs_create_quic_fec_ts_caster_listener(this, conf);
            if (!stream_caster_myproto_) {
                return srs_error_new(ERROR_STREAM_CASTER_ENGINE, "Failed to create quic_fec_ts caster listener");
            }
...
srs_error_t SrsServer::http_handle()
{
    ...
    if ((err = http_api_mux_->handle("/api/v1/adapters", adapter_api_handler_)) != srs_success) {
        return srs_error_wrap(err, "handle adapters");
    }
    if ((err = http_api_mux_->handle("/api/v1/adapters/", adapter_api_handler_)) != srs_success) {
        return srs_error_wrap(err, "handle adapters prefix");
    }
    if ((err = http_api_mux_->handle("/api/v1/adapters/stats", adapter_api_handler_)) != srs_success) {
        return srs_error_wrap(err, "handle adapters stats");
    }
