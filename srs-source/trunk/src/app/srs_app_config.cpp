...
bool srs_stream_caster_is_myproto(string caster)
{
    return caster == "myproto";
}

bool srs_stream_caster_is_quic_fec_ts(string caster)
{
    return caster == "quic_fec_ts";
}
...
srs_error_t SrsConfig::check_normal_config()
{
    ...
    for (int n = 0; n < (int)stream_casters.size(); n++) {
        ...
        for (int i = 0; stream_caster && i < (int)stream_caster->directives_.size(); i++) {
            ...
            string n = conf->name_;
            if (n != "enabled" && n != "caster" && n != "output" && n != "listen" && n != "sip" && n != "myproto") {
            ...
            if (n == "myproto") {
                for (int j = 0; j < (int)conf->directives_.size(); j++) {
                    string m = conf->at(j)->name_;
                    if (m != "listen" && m != "output" && m != "route" && m != "hot_start" && m != "params") {
                        return srs_error_new(ERROR_SYSTEM_CONFIG_INVALID, "illegal stream_caster.myproto.%s", m.c_str());
                    }
                }
            }
        }
